/**
 * Kanucard Landing Page - Main JavaScript
 */
...
(function($) {
    ...
    function initContactForm() {
        ...
        $form.on('submit', function(e) {
            ...
            // Ajax送信
            $.ajax({
                url: kanucard_ajax.ajax_url || '/wp-admin/admin-ajax.php',
                type: 'POST',
                data: formData,
    ...
    function initCounterAnimation() {
                ...
                $({ countNum: 0 }).animate({
                    ...
                    complete: function() {
                        ...
                        if ($this.parent().find('.stat-label').text().includes('%')) {
                            $this.text(this.countNum + '%');
                        } else if ($this.parent().find('.stat-label').text().includes('年')) {
                            $this.text(this.countNum + '+');
                        } else {
        ...
        $(window).on('scroll', function() {
            if (!animated && isElementInViewport($('.stats-section'))) {
                animated = true;
                animateCounters();
            }
        });
        ...
        // 初期チェック
        if (isElementInViewport($('.stats-section'))) {
            animated = true;
            animateCounters();
        }
    }
